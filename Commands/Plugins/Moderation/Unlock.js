module.exports = {
  ...
  category: "Mod",
  explanation:
    "Enable sending messages for the specified role/user or the guild. Only executable by someone with the manage channels permissions.",
  user: "Manage Channels",
  bot: "Manage Channels",
  usage: "unlock [role/user]",
  example: "unlock bot-users",
  explain:
    "Unlocks the channel for the role **bot-users**. If it is a user, then it is unlocked for a user, else for the guild.",
  code: `$if[$memberexists[$findmember[$message[1];no]]==true]
$if[$serverchannelexists[$getservervar[bl]]==true]
$channelsendmessage[$getservervar[bl];{author:A channel was unlocked for a user!:$authoravatar}{description:<@$authorid> unlocked <#$channelid>, for the __user__ <@$findmember[$message[1];no]>!\n\n[Jump To Message](https://discord.com/channels/$guildid/$channelid/$messageid)}{color:$getServerVar[color]}{footer:$username[$clientid] bot-mod logs}{timestamp}{thumbnail:$useravatar[$get[user]]}]
$else
$endif
...
$onlyif[$replacetext[$replacetext[$checkcondition[$authorid==$ownerid];true;1];false;$roleposition[$highestrole[$clientid]]]<$replacetext[$replacetext[$checkcondition[$authorid==$ownerid];true;2];false;$roleposition[$findrole[$message]]];{description:The role is **above/equal to me** in position!}{color:$GetServervar[color]}]
$onlyif[$replacetext[$replacetext[$checkcondition[$authorid==$ownerid];true;1];false;$roleposition[$highestrole[$clientid]]]<$replacetext[$replacetext[$checkcondition[$authorid==$ownerid];true;2];false;$roleposition[$findrole[$message]]];{description:The role is **above/equal to you** in position!}{color:$GetServervar[color]}]
$endelseif
$else
...
$log[[DEBUG] :: $username, used the unlock command]
`,
};
